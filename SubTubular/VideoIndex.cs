...
public sealed class VideoIndexRepository
{
    ...
    internal VideoIndex Build(string key)
    {
        ...
        FullTextIndex<string> index = builder.WithIndexModificationAction(async indexSnapshot => await SaveAsync(indexSnapshot, key)).Build();
...
internal sealed class VideoIndex
{
        ...
        [EnumeratorCancellation] CancellationToken cancellation = default)
    {
        ...
        foreach (var match in matches)
        {
            ...
            var captionTrackMatches = match.FieldMatches.Where(m => m.FoundIn.EndsWith(CaptionTrack.FieldSuffix));
            ...
            if (captionTrackMatches.Any()) result.MatchingCaptionTracks = captionTrackMatches.Select(m =>
            {
                ...
                var matches = new MatchedText(track.GetFullText(), m.Locations
                    //.OrderBy(x => x.Start) // return matches in order
